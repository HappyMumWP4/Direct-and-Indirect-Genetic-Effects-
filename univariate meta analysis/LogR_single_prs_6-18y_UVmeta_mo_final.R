...
output_dir <- "logr_single_prs_6-18y_mo_UVmeta_FE"  # Change directory name according to the statistical model
    ...
    if (adjusted_pvals_fdr_fe[i] < 0.05) {
      ...
      g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
        ...
        scale_size_continuous(
          ...
          breaks = c(100, 200, 500, 1000, 2000, 4000, 6000),  # Define custom breaks
          labels = c("100", "200", "500", "1K", "2K", "4K", "6K"),  # Format labels
          limits = c(100, 6000)  # Ensure the full range is considered
        ) + 
...
# Define the directory where you want to save the forest plot PNG files
output_dir <- "logr_single_prs_6-18y_mo_UVmeta_RE"  # Change directory name according to the statistical model
...
save(meta_results, file = "LogRM_single_prs_6-18y_mo_UVmeta.RData")

cat("Combined data and output results saved to 'LogRM_single_prs_6-18y_mo_UVmeta.RData'.\n") # Change file name according to the statistical model
...
write_csv(combined_data, "logr_single_prs_6-18y_mo_combined_studies.csv") # Change file name according to the statistical model

cat("Combined data saved to 'logr_single_prs_6-18y_mo_combined_studies.csv'.\n") # Change file name according to the statistical model
