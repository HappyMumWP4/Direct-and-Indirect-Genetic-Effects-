...
output_dir <- "lrm_trio_prs_1-5y_mo_UVmeta_FE"  # Change directory name according to the statistical model
...
perform_meta_analysis_and_plot_FE <- function(outcome, predictors_pattern, output_dir) {
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    if (adjusted_pvals_fdr_fe[i] < 0.05) {
      ...
      # Define output file
      file_name <- paste0("UVFE_lrm_trio_", short_predictor, "_", short_outcome, "_mo", ".png") # Change file name according to the statistical model
      ...
      g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
        ...
        scale_size_continuous(
          ...
          breaks = c(100, 200, 500, 1000, 2000, 5000, 10000, 20000, 30000),  # Define custom breaks
          labels = c("100", "200", "500", "1K", "2K", "5K", "10K", "20K", "30K"),  # Format labels
          limits = c(100, 30000)  # Ensure the full range is considered
        ) + 
        
        labs(x = "Beta Coefficient", y = "Study", 
             title = paste("Forest Plot (FE) of Trio-LRM for", outcome, "and", predictor),
             size = "Sample Size") +
  ...
  csv_file_path <- file.path(output_dir, paste0("lrm_trio_", short_outcome, "_", predictors_pattern, "UVmeta_FE_mo.csv")) # Change file name according to the statistical model
...
# Define the directory where you want to save the forest plot PNG files
output_dir <- "lrm_trio_prs_1-5y_mo_UVmeta_RE"  # Change directory name according to the statistical model
...
perform_meta_analysis_and_plot_RE <- function(outcome, predictors_pattern, output_dir) {
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    if (adjusted_pvals_fdr_re[i] < 0.05) {
      ...
      # Define output file
      file_name <- paste0("UVRE_lrm_trio_", short_predictor, "_", short_outcome, "_mo", ".png") # Change file name according to the statistical model
      file_path <- file.path(output_dir, file_name)
      ...
      g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
        ...
        labs(x = "Beta Coefficient", y = "Study", 
             title = paste("Forest Plot (RE) of Trio-LRM for", outcome, "and", predictor),
             size = "Sample Size") +
  ...
  csv_file_path <- file.path(output_dir, paste0("lrm_trio_", short_outcome, "_", predictors_pattern, "UVmeta_RE_mo.csv")) # Change file name according to the statistical model
...
save(meta_results, file = "LRM_trio_prs_1-5y_mo_UVmeta.RData")

cat("Combined data and output results saved to 'LRM_trio_prs_1-5y_mo_UVmeta.RData'.\n") # Change file name according to the statistical model
...
write_csv(combined_data, "lrm_trio_prs_1-5y_mo_combined_studies.csv") # Change file name according to the statistical model

cat("Combined data saved to 'lrm_trio_prs_1-5y_mo_combined_studies.csv'.\n") # Change file name according to the statistical model
