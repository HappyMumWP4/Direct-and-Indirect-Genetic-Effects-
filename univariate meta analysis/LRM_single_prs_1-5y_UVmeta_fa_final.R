...
output_dir <- "lrm_single_prs_1-5y_fa_UVmeta_FE"  # Change directory name according to the statistical model
    ...
    if (adjusted_pvals_fdr_fe[i] < 0.05) {
      ...
      g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
        ...
        scale_size_continuous(
          ...
          breaks = c(50, 100, 200, 1000, 2000, 4000),  # Define custom breaks
          labels = c("50", "100", "200", "1K", "2K", "4K"),  # Format labels
...
# Define the directory where you want to save the forest plot PNG files
output_dir <- "lrm_single_prs_1-5y_fa_UVmeta_RE"  # Change directory name according to the statistical model
...
save(meta_results, file = "LRM_single_prs_1-5y_fa_UVmeta.RData")

cat("Combined data and output results saved to 'LRM_single_prs_1-5y_fa_UVmeta.RData'.\n") # Change file name according to the statistical model
...
write_csv(combined_data, "lrm_single_prs_1-5y_fa_combined_studies.csv") # Change file name according to the statistical model

cat("Combined data saved to 'lrm_single_prs_1-5y_fa_combined_studies.csv'.\n") # Change file name according to the statistical model
