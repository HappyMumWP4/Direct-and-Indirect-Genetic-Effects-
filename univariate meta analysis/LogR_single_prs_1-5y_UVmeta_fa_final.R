...
output_dir <- "logr_single_prs_1-5y_fa_UVmeta_FE"  # Change directory name according to the statistical model
...
perform_meta_analysis_and_plot_FE <- function(predictors_pattern, output_dir) {
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    if (adjusted_pvals_fdr_fe[i] < 0.05) {
      ...
      # Define output file
      file_name <- paste0("UVFE_logr_", short_predictor, "_fa", ".png") # Change file name according to the statistical model
      ...
      g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
        ...
        scale_size_continuous(
          ...
          breaks = c(50, 100, 200, 500, 1000, 2000, 4000),  # Define custom breaks
          labels = c("50", "100", "200", "500", "1K", "2K", "4K"),  # Format labels
          limits = c(50, 4000)  # Ensure the full range is considered
        ) + 
  ...
  csv_file_path <- file.path(output_dir, paste0("logr_", predictors_pattern, "UVmeta_FE_fa.csv")) # Change file name according to the statistical model
...
# Define the directory where you want to save the forest plot PNG files
output_dir <- "logr_single_prs_1-5y_fa_UVmeta_RE"  # Change directory name according to the statistical model
...
perform_meta_analysis_and_plot_RE <- function(predictors_pattern, output_dir) {
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    if (adjusted_pvals_fdr_re[i] < 0.05) {
      ...
      # Define output file
      file_name <- paste0("UVRE_logr_", short_predictor, "_fa", ".png") # Change file name according to the statistical model
      file_path <- file.path(output_dir, file_name)
  ...
  csv_file_path <- file.path(output_dir, paste0("logr_", predictors_pattern, "UVmeta_RE_fa.csv")) # Change file name according to the statistical model
...
save(meta_results, file = "LogRM_single_prs_1-5y_fa_UVmeta.RData")

cat("Combined data and output results saved to 'LogRM_single_prs_1-5y_fa_UVmeta.RData'.\n") # Change file name according to the statistical model
...
write_csv(combined_data, "logr_single_prs_1-5y_fa_combined_studies.csv") # Change file name according to the statistical model

cat("Combined data saved to 'logr_single_prs_1-5y_fa_combined_studies.csv'.\n") # Change file name according to the statistical model
