...
output_dir <- "logr_trio_prs_6-18y_mo_UVmeta_FE"  # Change directory name according to the statistical model
...
perform_meta_analysis_and_plot_FE <- function(predictors_pattern, output_dir) {
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    if (adjusted_pvals_fdr_fe[i] < 0.05) {
      ...
      file_name <- paste0("UVFE_logr_trio_", short_predictor, "_mo", ".png") # Change file name according to the statistical model
      ...
      g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
        ...
        labs(x = "Beta Coefficient", y = "Study", 
             title = paste("Forest Plot (FE) of Trio-LogRM for", predictor),
             size = "Sample Size") +
  ...
  csv_file_path <- file.path(output_dir, paste0("logr_trio_", predictors_pattern, "UVmeta_FE_mo.csv")) # Change file name according to the statistical model
...
# Perform meta-analysis for each set of predictors
patterns <- c("prs_c_qc_", "prs_m_qc_", "prs_f_qc_")
...
# Define the directory where you want to save the forest plot PNG files
output_dir <- "logr_trio_prs_6-18y_mo_UVmeta_RE"  # Change directory name according to the statistical model
...
perform_meta_analysis_and_plot_RE <- function(predictors_pattern, output_dir) {
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    if (adjusted_pvals_fdr_re[i] < 0.05) {
      ...
      # Define output file name
      file_name <- paste0("UVRE_logr_trio_", short_predictor, "_mo", ".png") # Change file name according to the statistical model
      file_path <- file.path(output_dir, file_name)
      ...
      g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
        ...
        labs(x = "Beta Coefficient", y = "Study", 
             title = paste("Forest Plot (RE) of Trio-LogRM for", predictor),
             size = "Sample Size") +
  ...
  csv_file_path <- file.path(output_dir, paste0("logr_trio_", predictors_pattern, "UVmeta_RE_mo.csv")) # Change file name according to the statistical model
...
save(meta_results, file = "LogRM_trio_prs_6-18y_mo_UVmeta.RData")

cat("Combined data and output results saved to 'LogRM_trio_prs_6-18y_mo_UVmeta.RData'.\n") # Change file name according to the statistical model
...
write_csv(combined_data, "logr_trio_prs_6-18y_mo_combined_studies.csv") # Change file name according to the statistical model

cat("Combined data saved to 'logr_trio_prs_6-18y_mo_combined_studies.csv'.\n") # Change file name according to the statistical model
