...
for (file in file_list) {
  ...
  if (all(c("CBCL_outcome", "PRS_predictor", "Beta", "SE", "N", "CI_lower", "CI_upper", "Odds_ratio") %in% colnames(data))) {
    ...
    # Convert confidence intervals and odds ratio to log scale
    data <- data %>%
      mutate(
        CI_lower = log(CI_lower),  # Log-transformed CI lower bound
        CI_upper = log(CI_upper),  # Log-transformed CI upper bound
        # `log(OR)` = log(Odds_ratio),  # Log-transformed Odds Ratio
        # Std_error = (CI_upper - CI_lower) / (2 * 1.96)  # Recalculated Standard Error
      )
...
output_dir <- "logr_single_prs_1-5y_mo_UVmeta_FE"  # Change directory name according to the statistical model
...
perform_meta_analysis_and_plot_FE <- function(predictors_pattern, output_dir) {
  
  # Get the unique predictors that match the specified pattern
  unique_predictors <- unique(grep(predictors_pattern, combined_data$PRS_predictor, value = TRUE))
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    predictor_data <- subset(combined_data, PRS_predictor == predictor)
    ...
    FE_IV_results_df <- rbind(FE_IV_results_df, data.frame(
      Outcome = as.character(unique(predictor_data$CBCL_outcome)),     # Add the CBCL outcome
      Predictor = predictor,
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    if (adjusted_pvals_fdr_fe[i] < 0.05) {
      ...
      # Filter combined_data for the selected predictor
      outcome_df <- combined_data %>%
      ...
      # Get UVMA result
      uvma_results <- FE_IV_results_df %>% filter(Predictor == predictor)
      
      if (nrow(uvma_results) == 0) {
        message("No UVMA results for predictor. ", "Skipping plot.")
        next
      }
      ...
      print(plot_data)  # Debug: Check data frame contents
      ...
      # Define output file
      file_name <- paste0("UVFE_logr_", short_predictor, "_mo", ".png") # Change file name according to the statistical model
      ...
      g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
        ...
        scale_size_continuous(
          ...
          breaks = c(100, 200, 500, 1000, 2000, 3000, 4000, 5000),  # Define custom breaks
          labels = c("100", "200", "500", "1K", "2K", "3K", "4K", "5K"),  # Format labels
          limits = c(100, 5000)  # Ensure the full range is considered
        ) + 
        
        labs(x = "Beta Coefficient", y = "Study", 
             title = paste("Forest Plot (FE) of Single-LogRM for", predictor),
             size = "Sample Size") +
  ...
  csv_file_path <- file.path(output_dir, paste0("logr_", predictors_pattern, "UVmeta_FE_mo.csv")) # Change file name according to the statistical model
...
# Perform meta-analysis for each CBCL_outcome and each set of predictors
patterns <- c("prs_c_qc_", "prs_m_qc_", "prs_f_qc_", "prs_nt_qc_", "pTDTd_qc_")

# Initialize a list to store results for all predictor patterns
all_results <- list()

for (pattern in patterns) {
  result_df <- perform_meta_analysis_and_plot_FE(pattern, output_dir)  
  all_results[[pattern]] <- result_df  # Store each result in a list
}

# Combine all results into a single data frame
FE_final_results_df <- bind_rows(all_results, .id = "PredictorPattern")
...
# Define the directory where you want to save the forest plot PNG files
output_dir <- "logr_single_prs_1-5y_mo_UVmeta_RE"  # Change directory name according to the statistical model
...
perform_meta_analysis_and_plot_RE <- function(predictors_pattern, output_dir) {
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    if (adjusted_pvals_fdr_re[i] < 0.05) {
      ...
      # Get UVMA result
      uvma_results <- RE_IV_results_df %>% filter(Predictor == predictor)
      ...
      # Define output file
      file_name <- paste0("UVRE_logr_", short_predictor, "_mo", ".png") # Change file name according to the statistical model
      file_path <- file.path(output_dir, file_name)
      ...
      g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
        ...
        labs(x = "Beta Coefficient", y = "Study", 
             title = paste("Forest Plot (RE) of Single-LogRM for", predictor),
             size = "Sample Size") +
  ...
  csv_file_path <- file.path(output_dir, paste0("logr_", predictors_pattern, "UVmeta_RE_mo.csv")) # Change file name according to the statistical model
...
for (pattern in patterns) {
  result_df <- perform_meta_analysis_and_plot_RE(pattern, output_dir)  
  all_results[[pattern]] <- result_df  # Store each result in a list
}

# Combine all results into a single data frame
RE_final_results_df <- bind_rows(all_results, .id = "PredictorPattern")
...
save(meta_results, file = "LogRM_single_prs_1-5y_mo_UVmeta.RData")

cat("Combined data and output results saved to 'LogRM_single_prs_1-5y_mo_UVmeta.RData'.\n") # Change file name according to the statistical model
...
write_csv(combined_data, "logr_single_prs_1-5y_mo_combined_studies.csv") # Change file name according to the statistical model

cat("Combined data saved to 'logr_single_prs_1-5y_mo_combined_studies.csv'.\n") # Change file name according to the statistical model
