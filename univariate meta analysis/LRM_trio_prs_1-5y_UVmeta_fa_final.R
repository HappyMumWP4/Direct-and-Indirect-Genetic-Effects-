...
output_dir <- "lrm_trio_prs_1-5y_fa_UVmeta_FE"  # Change directory name according to the statistical model
...
perform_meta_analysis_and_plot_FE <- function(outcome, predictors_pattern, output_dir) {
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    if (adjusted_pvals_fdr_fe[i] < 0.05) {
      ...
      # Define output file
      file_name <- paste0("UVFE_lrm_trio_", short_predictor, "_", short_outcome, "_fa", ".png") # Change file name according to the statistical model
      ...
      g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
        ...
        scale_size_continuous(
          ...
          breaks = c(50, 100, 1000, 2000, 4000),  # Define custom breaks
          labels = c("50", "100", "1K", "2K", "4K"),  # Format labels
  ...
  csv_file_path <- file.path(output_dir, paste0("lrm_trio_", short_outcome, "_", predictors_pattern, "UVmeta_FE_fa.csv")) # Change file name according to the statistical model
...
# Define the directory where you want to save the forest plot PNG files
output_dir <- "lrm_trio_prs_1-5y_fa_UVmeta_RE"  # Change directory name according to the statistical model
...
perform_meta_analysis_and_plot_RE <- function(outcome, predictors_pattern, output_dir) {
  ...
  for (i in seq_along(unique_predictors)) {
    ...
    if (adjusted_pvals_fdr_re[i] < 0.05) {
      ...
      # Define output file
      file_name <- paste0("UVRE_lrm_trio_", short_predictor, "_", short_outcome, "_fa", ".png") # Change file name according to the statistical model
      file_path <- file.path(output_dir, file_name)
  ...
  csv_file_path <- file.path(output_dir, paste0("lrm_trio_", short_outcome, "_", predictors_pattern, "UVmeta_RE_fa.csv")) # Change file name according to the statistical model
...
save(meta_results, file = "LRM_trio_prs_1-5y_fa_UVmeta.RData")

cat("Combined data and output results saved to 'LRM_trio_prs_1-5y_fa_UVmeta.RData'.\n") # Change file name according to the statistical model
...
write_csv(combined_data, "lrm_trio_prs_1-5y_fa_combined_studies.csv") # Change file name according to the statistical model

cat("Combined data saved to 'lrm_trio_prs_1-5y_fa_combined_studies.csv'.\n") # Change file name according to the statistical model
