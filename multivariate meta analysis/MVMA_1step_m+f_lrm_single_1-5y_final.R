...
output_dir <- "MVMA_FE_1step_m+f_lrm_sng_all_1-5y"
...
rho_values <- list(
  "h_c_total_1_5y" = c("GenR3" = 0.46, "GenR4" = 0.435, "NTR" = 0.681),
  "h_c_extern_1_5y" = c("GenR3" = 0.53, "GenR4" = 0.428, "NTR" = 0.695),
  "h_c_intern_1_5y" = c("GenR3" = 0.481, "GenR4" = 0.441, "NTR" = 0.652)
)
...
full_studies <- c("fa_GenR3", "mo_GenR3", "fa_GenR4", "mo_GenR4",
                  "fa_NTR", "mo_NTR", "mo_MoBa", "mix_MCS", 
                  "mo_POSEIDON", "mo_PREDO", "fa_TwinLife", "mo_TwinLife")  # 12 studies, change if necessary!

# Define studies available for ANX (NTR missing)
anx_studies <- c("fa_GenR3", "mo_GenR3", "fa_GenR4", "mo_GenR4",
                 "mo_MoBa", "mix_MCS", "mo_POSEIDON", "mo_PREDO", 
                 "fa_TwinLife", "mo_TwinLife")  # 10 studies (no NTR), change if necessary!
...
for (key in names(fdr_corrected_results)) {
  ...
  for (i in seq_len(nrow(sig_df))) {
    ...
    cohort_order <- c("ALSPAC", "GenR3", "GenR4", "MCS", "MoBa", 
    ...
    # Output file
    file_name <- paste0("MVMA_FE_lrm_sng_", short_predictor, "_", short_outcome, ".png")
    ...
    g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
      ...
      scale_size_continuous(
        range = c(3, 8),
        breaks = c(50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 33000),
        labels = c("50", "100", "200", "500", "1K", "2K", "5K", "10K", "20K", "33K"),
        limits = c(50, 35000)
      ) +
...
# Save results to CSV
output_file <- file.path(output_dir, "MVMA_FE_1step_lrm_sng_all_1-5y.csv")
...
save(meta_results, file = file.path(output_dir, "MVMA_FE_1step_lrm_sng_all_1-5y.RData")) # Change file name according to the model and cohort
 
cat("Meta-analysis objects saved as 'MVMA_FE_1step_lrm_sng_all_1-5y.RData'.\n")
...
# Define output directory
output_dir <- "MVMA_RE_1step_m+f_lrm_sng_all_1-5y"
...
for (key in names(fdr_corrected_results)) {
  ...
  for (i in seq_len(nrow(sig_df))) {
    ...
    # Output file
    file_name <- paste0("MVMA_RE_lrm_sng_", short_predictor, "_", short_outcome, ".png")
    file_path <- file.path(output_dir, file_name)
...
# Save results to CSV
output_file <- file.path(output_dir, "MVMA_RE_1step_lrm_sng_all_1-5y.csv")
write.csv(final_results, file = output_file, row.names = FALSE)

cat("Final results saved to", output_file, "\n")

# Save the meta-analysis models & statistics for future use
save(meta_results, file = file.path(output_dir, "MVMA_RE_1step_lrm_sng_all_1-5y.RData")) # Change file name according to the model and cohort

cat("Meta-analysis objects saved as 'MVMA_RE_1step_lrm_sng_all_1-5y.RData'.\n")
...
write.csv(meta_data, "lrm_sng_1-5y_m+f_combined.csv") # Change file name according to the model and cohort

cat("Combined data saved to 'lrm_sng_1-5y_m+f_combined.csv'.\n")
