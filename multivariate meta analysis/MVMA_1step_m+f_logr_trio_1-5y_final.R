...
output_dir <- "MVMA_FE_1step_m+f_logr_trio_all_1-5y"
...
rho_values <- list("h_c_total_cat_1_5y" = c("GenR3" = 0.46, "NTR" = 0.681))
...
# Define full study lists
full_studies <- c("fa_GenR3", "mo_GenR3", "fa_GenR4", "fa_NTR", "mo_NTR", "mix_MCS", 
                  "mo_POSEIDON", "mo_PREDO", "fa_TwinLife", "mo_TwinLife")  # 10 studies, change if necessary!

# Define studies available for ANX (NTR missing)
anx_studies <- c("fa_GenR3", "mo_GenR3", "fa_GenR4", "mix_MCS", 
                 "mo_POSEIDON", "mo_PREDO", "fa_TwinLife", "mo_TwinLife")  # 8 studies (no NTR), change if necessary!
...
for (outcome in names(rho_values)) {
  ...
  for (prs in prs_predictors) {
    ...
    if (grepl("_qc_ANX_trio", prs)) {
      ...
      required_studies <- 8 # Define selected number studies for certain prs!
    } else {
      study_list <- full_studies
      required_studies <- 10 # Define full number of studies! 
    }
    ...
    for (group in names(correlated_pairs)) {
      ...
      if (all(pair %in% meta_subset$StudyName)) {
        rho <- rho_between[group]
        idx1 <- which(meta_subset$StudyName == pair[1])
        idx2 <- which(meta_subset$StudyName == pair[2])
        cov_value <- rho * meta_subset$SE[idx1] * meta_subset$SE[idx2]
        V_matrix[idx1, idx2] <- cov_value
        V_matrix[idx2, idx1] <- cov_value
      }
    }
...
for (key in names(fdr_corrected_results)) {
  ...
  for (i in seq_len(nrow(sig_df))) {
    ...
    cohort_order <- c("GenR3", "GenR4", "MCS", 
      "NTR", "POSEIDON", "PREDO", "TwinLife")              # Change study names if necessary!
    ...
    # Output file
    file_name <- paste0("MVMA_FE_logr_trio_", short_predictor, "_", short_outcome, ".png")
    ...
    g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
      ...
      scale_size_continuous(
        ...
        labels = c("50", "100", "200", "500", "1K", "2K", "5K", "10K"),
        limits = c(50, 12000)
      ) +
      labs(x = "Beta Coefficient", y = "Study",
           title = paste("Forest Plot (FE) of Trio-logr for", outcome, "and", prs),
           size = "Sample Size") +
...
# Save results to CSV
output_file <- file.path(output_dir, "MVMA_FE_1step_logr_trio_all_1-5y.csv")
...
save(meta_results, file = file.path(output_dir, "MVMA_FE_1step_logr_trio_all_1-5y.RData")) # Change file name according to the model and cohort
 
cat("Meta-analysis objects saved as 'MVMA_FE_1step_logr_trio_all_1-5y.RData'.\n")
...
# Define output directory
output_dir <- "MVMA_RE_1step_m+f_logr_trio_all_1-5y"
...
for (key in names(fdr_corrected_results)) {
  ...
  for (i in seq_len(nrow(sig_df))) {
    ...
    # Output file
    file_name <- paste0("MVMA_RE_logr_trio_", short_predictor, "_", short_outcome, ".png")
    file_path <- file.path(output_dir, file_name)
    ...
    g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
      ...
      labs(x = "Beta Coefficient", y = "Study",
           title = paste("Forest Plot (RE) of Trio-logr for", outcome, "and", prs),
           size = "Sample Size") +
      theme_minimal(base_size = 14) +
      theme(plot.title = element_text(size = 14, hjust = 0.5, face = "bold"),
            plot.margin = margin(10, 10, 10, 10))
...
# Save results to CSV
output_file <- file.path(output_dir, "MVMA_RE_1step_logr_trio_all_1-5y.csv")
write.csv(final_results, file = output_file, row.names = FALSE)

cat("Final results saved to", output_file, "\n")

# Save the meta-analysis models & statistics for future use
save(meta_results, file = file.path(output_dir, "MVMA_RE_1step_logr_trio_all_1-5y.RData")) # Change file name according to the model and cohort

cat("Meta-analysis objects saved as 'MVMA_RE_1step_logr_trio_all_1-5y.RData'.\n")
...
write.csv(meta_data, "logr_trio_1-5y_m+f_combined.csv") # Change file name according to the model and cohort

cat("Combined data saved to 'logr_trio_1-5y_m+f_combined.csv'.\n")
