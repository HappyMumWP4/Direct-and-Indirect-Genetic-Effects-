...
output_dir <- "MVMA_FE_1step_m+f_logr_sng_all_6-18y"
...
for (key in names(fdr_corrected_results)) {
  ...
  for (i in seq_len(nrow(sig_df))) {
    ...
    # Output file
    file_name <- paste0("MVMA_FE_logr_sng_", short_predictor, "_", short_outcome, ".png")
...
# Save results to CSV
output_file <- file.path(output_dir, "MVMA_FE_1step_logr_sng_all_6-18y.csv")
...
save(meta_results, file = file.path(output_dir, "MVMA_FE_1step_logr_sng_all_6-18y.RData")) # Change file name according to the model and cohort
 
cat("Meta-analysis objects saved as 'MVMA_FE_1step_logr_sng_all_6-18y.RData'.\n")
...
# Define output directory
output_dir <- "MVMA_RE_1step_m+f_logr_sng_all_6-18y"
...
for (key in names(fdr_corrected_results)) {
  ...
  for (i in seq_len(nrow(sig_df))) {
    ...
    # Output file
    file_name <- paste0("MVMA_RE_logr_sng_", short_predictor, "_", short_outcome, ".png")
    file_path <- file.path(output_dir, file_name)
...
# Save results to CSV
output_file <- file.path(output_dir, "MVMA_RE_1step_logr_sng_all_6-18y.csv")
write.csv(final_results, file = output_file, row.names = FALSE)

cat("Final results saved to", output_file, "\n")

# Save the meta-analysis models & statistics for future use
save(meta_results, file = file.path(output_dir, "MVMA_RE_1step_logr_sng_all_6-18y.RData")) # Change file name according to the model and cohort

cat("Meta-analysis objects saved as 'MVMA_RE_1step_logr_sng_all_6-18y.RData'.\n")
...
write.csv(meta_data, "logr_sng_6-18y_m+f_combined.csv") # Change file name according to the model and cohort

cat("Combined data saved to 'logr_sng_6-18y_m+f_combined.csv'.\n")
