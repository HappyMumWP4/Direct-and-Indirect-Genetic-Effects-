...
output_dir <- "MVMA_FE_1step_m+f_logr_trio_all_6-18y"
...
rho_values <- list("h_c_total_cat_6_18y" = c("GenR3" = 0.402, "GenR4" = 0.38, "NTR" = 0.698))
...
# Define study lists
full_studies <- c("fa_GenR3", "mo_GenR3", "fa_GenR4", "mo_GenR4", "mix_MCS", 
                  "fa_NTR", "mo_NTR", "mo_POSEIDON", "mo_PREDO")
...
for (outcome in names(rho_values)) {
  ...
  for (prs in prs_predictors) {
    ...
    if (grepl("_qc_ANX_trio", prs)) {
      ...
      required_studies <- 7 # Define selected number studies for certain prs!
    } else {
      study_list <- full_studies
      required_studies <- 9 # Define full number of studies! 
    }
  ...
  for (i in seq_len(nrow(sig_df))) {
    ...
    g <- ggplot(plot_data, aes(y = StudyName, x = Beta, xmin = CI_lower, xmax = CI_upper)) +
      ...
      scale_size_continuous(
        ...
        labels = c("100", "200", "500", "1K", "2K", "5K", "10K"),
        limits = c(100, 12000)
      ) +
...
# Save results to CSV
output_file <- file.path(output_dir, "MVMA_FE_1step_logr_trio_all_6-18y.csv")
...
save(meta_results, file = file.path(output_dir, "MVMA_FE_1step_logr_trio_all_6-18y.RData")) # Change file name according to the model and cohort
 
cat("Meta-analysis objects saved as 'MVMA_FE_1step_logr_trio_all_6-18y.RData'.\n")
...
# Define output directory
output_dir <- "MVMA_RE_1step_m+f_logr_trio_all_6-18y"
...
# Save results to CSV
output_file <- file.path(output_dir, "MVMA_RE_1step_logr_trio_all_6-18y.csv")
write.csv(final_results, file = output_file, row.names = FALSE)

cat("Final results saved to", output_file, "\n")

# Save the meta-analysis models & statistics for future use
save(meta_results, file = file.path(output_dir, "MVMA_RE_1step_logr_trio_all_6-18y.RData")) # Change file name according to the model and cohort

cat("Meta-analysis objects saved as 'MVMA_RE_1step_logr_trio_all_6-18y.RData'.\n")
...
write.csv(meta_data, "logr_trio_6-18y_m+f_combined.csv") # Change file name according to the model and cohort

cat("Combined data saved to 'logr_trio_6-18y_m+f_combined.csv'.\n")
